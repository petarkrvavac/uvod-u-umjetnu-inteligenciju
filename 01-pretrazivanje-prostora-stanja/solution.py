"""
Laboratorijska vježba 1 - Pretraživanje prostora stanja
...
def reconstruct_path(parent, start_state, goal_state):
    """
    Rekonstruira putanju od početnog do ciljnog stanja.

    Args:
        parent: dict - rječnik roditelja {stanje: prethodno_stanje}
        start_state: str - početno stanje
        goal_state: str - ciljno stanje

    Returns:
        list - lista stanja od početnog do ciljnog
    """
    path = []
    current = goal_state
    # Kreći se unazad od cilja prema početku
    while current is not None:
        path.append(current)
        current = parent.get(current)
    # Obrni putanju da ide od početka prema cilju
    path.reverse()
    return path
...
def shortest_path_cost(start, goals, graph):
    """
    Izračunava stvarnu cijenu najkraćeg puta (h*) od početnog stanja do najbližeg cilja.
    Koristi UCS/Dijkstra algoritam za pronalaženje optimalnog puta.

    Args:
        start: str - početno stanje
        goals: list - lista ciljnih stanja
        graph: dict - graf prijelaza {stanje: {susjed: cijena}}

    Returns:
        float - najjeftinija cijena do cilja ili inf ako cilj nije dostižan
    """
    # Rječnik najboljih poznatih cijena za svako stanje
    g_cost = {start: 0.0}
    # Priority queue: (cijena, stanje)
    pq = [(0.0, start)]
    # Skup već obrađenih stanja
    closed = set()
    # Konvertiraj goals u set za O(1) provjeru članstva
    goals_set = set(goals) if not isinstance(goals, set) else goals

    while pq:
        cost, state = heapq.heappop(pq)

        # Preskoči zastarjeli unos iz priority queue-a
        # (ako smo u međuvremenu našli jeftiniji put do ovog stanja)
        if cost > g_cost.get(state, float('inf')):
            continue

        # Preskoči ako je stanje već obrađeno
        if state in closed:
            continue
        closed.add(state)
        ...
        if state in goals_set:
            return cost  # Vraća h*(start) - stvarnu cijenu do cilja

        # Ekspandiraj susjede abecednim redom (za determinističko ponašanje)
        for neighbor, edge_cost in sorted(graph.get(state, {}).items()):
            tentative = cost + edge_cost

            # Ažuriraj ako smo našli jeftiniji put do susjeda
            if tentative < g_cost.get(neighbor, float('inf')):
                g_cost[neighbor] = tentative
                heapq.heappush(pq, (tentative, neighbor))

    # Cilj nije dostižan iz početnog stanja
    return float('inf')
...
def load_state_space(file_path):
    ...
    # Učitaj datoteku i filtriraj prazne redove i komentare
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = [line.strip() for line in f if line.strip() and not line.startswith('#')]
    except FileNotFoundError:
        print(f"Greška: Datoteka {file_path} nije pronađena.")
        sys.exit(1)

    if not lines:
        return None, [], {}

    # Parsiranje početnog stanja (1. redak)
    start_state = lines[0]
...
def load_heuristic(file_path):
    """
    Učitava heurističke vrijednosti iz datoteke.
    ...
    Returns:
        dict: {stanje: heuristička_vrijednost}
    """
    heuristics = {}
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
        ...
        # Vrati prazan rječnik ako datoteka ne postoji
        return {}
    except ValueError:
        # Vrati prazan rječnik ako je format neispravan
        return {}
...
def bfs(start, goals, graph):
    ...
    BFS istražuje prostor stanja sloj po sloj, počevši od početnog stanja.
    Prvo istražuje sva stanja na dubini 1, zatim na dubini 2, itd.
    Garantira najkraći put po broju čvorova (ne po cijeni).

    Koristi FIFO (First In First Out) red za obradu stanja.
    Susjedi se dodaju abecednim redom radi determinizma.

    Vremenska složenost: O(|V| + |E|) gdje je V broj stanja, E broj bridova
    Prostorna složenost: O(|V|) za čuvanje reda i skupa posjećenih stanja
    ...
    Returns:
        dict - rezultat pretraživanja s ključevima:
               found, visited, path_length, total_cost, path
    """
    # Skup već posjećenih stanja (sprječava ponavljanje)
    visited = {start}
    ...
    while queue:
        ...
        # Ovo osigurava da stanja budu obrađena po redoslijedu dubine
        state, path = queue.popleft()

        # Provjera cilja: jesmo li stigli do nekog od ciljnih stanja?
        # Provjeravamo nakon što izvučemo stanje iz reda (goal test on expansion)
        if state in goals_set:
            # Izračunaj ukupnu cijenu puta (suma svih cijena bridova)
            # Napomena: BFS ne garantira najjeftiniji put, samo najkraći po broju čvorova
            total_cost = 0.0
        ...
        # Ekspanzija: dodaj sve neposjećene susjede u red
        # Abecedno sortiranje susjeda (važno za BFS determinizam)
        if state not in graph:
            continue  # Nema susjeda, preskoči ekspanziju
        ...
        for neighbor in successors:
            if neighbor not in visited:
                ...
                # Dodaj susjeda u red s produženom putanjom
                queue.append((neighbor, path + [neighbor]))

    # Nismo pronašli rješenje
    return {"found": False, "visited": len(visited)}


def ucs_stable(start, goals, graph):
    """
    Uniform Cost Search (UCS) - Pretraživanje jednolične cijene.

    UCS istražuje prostor stanja tako da uvijek prvo ekspandira stanje s
    najmanjom ukupnom cijenom puta od početka. Garantira optimalan put
    ...
    Vremenska složenost: O(|E| log |V|) zbog operacija s priority queue-om
    Prostorna složenost: O(|V|) za čuvanje priority queue-a i closed skupa
    ...
    g_cost = {start: 0.0}
    # Rječnik roditelja za rekonstrukciju puta
    parent = {start: None}
    # Konvertiraj goals u set za O(1) provjeru članstva
    goals_set = set(goals) if not isinstance(goals, set) else goals

    # Priority queue: (g_cost, state)
    # state služi kao abecedni tie-breaker kada su g_cost vrijednosti jednake
    # Python automatski uspoređuje elemente tuple-a slijeva nadesno
    pq = [(0.0, start)]
    ...
    while pq:
        ...
        # Preskoči zastarjeli unos iz priority queue-a
        # (ako smo u međuvremenu našli jeftiniji put)
        if cost > g_cost.get(state, float('inf')):
            continue

        # Preskoči već obrađena stanja
        if state in closed:
            continue
        closed.add(state)

        # Provjera cilja: jesmo li pronašli rješenje?
        if state in goals_set:
            # Rekonstruiraj putanju od početka do cilja
        ...
        # Abecedno sortiranje za determinističko ponašanje
        if state not in graph:
            continue  # Nema susjeda, preskoči ekspanziju
...
def astar_stable(start, goals, graph, heuristics):
    """
    A* Search - Heuristički algoritam pretraživanja.

    A* kombinira stvarnu cijenu puta g(n) s heurističkom procjenom h(n)
    do cilja. Ekspandira stanje s najmanjom vrijednosti f(n) = g(n) + h(n).

    Ako je heuristika admisibilna (optimistična), A* garantira optimalan put.
    Ako je heuristika konzistentna, A* je još učinkovitiji.

    Koristi priority queue gdje je prioritet f(n) = g(n) + h(n).
    Abecedno sortiranje susjeda osigurava determinizam kod jednakih f(n).

    Vremenska složenost: O(|E| log |V|), ali često brže od UCS zbog heuristike
    Prostorna složenost: O(|V|) za čuvanje priority queue-a i closed skupa
    ...
    Args:
        ...
        goals: list - lista ciljnih stanja
        graph: dict - graf prijelaza {stanje: {susjed: cijena}}
        heuristics: dict - heurističke vrijednosti {stanje: h(stanje)}
    ...
    # Rječnik roditelja za rekonstrukciju puta
    parent = {start: None}
    # Heuristička vrijednost početnog stanja
    ...
    while pq:
        ...
        if state in goals_set:
            ...
            return {
                ...
                "visited": len(closed),
                "path_length": len(path),
                "total_cost": g,  # Vraćamo g (stvarnu cijenu), ne f
                "path": path
            }
        ...
        for neighbor, edge_cost in sorted(graph[state].items()):
            # Izračunaj novu g(neighbor) vrijednost
            ...
            if tentative_g < g_cost.get(neighbor, float('inf')):
                ...
                parent[neighbor] = state
                # Dohvati heurističku procjenu za susjeda
                h = heuristics.get(neighbor, 0.0)
                # Izračunaj f(neighbor) = g(neighbor) + h(neighbor)
...
def check_optimistic(graph, heuristics, goals, heuristic_file):
    """
    Provjerava je li heuristika optimistična (admisibilna).

    Heuristika h je optimistična ako za svako stanje s vrijedi:
    h(s) <= h*(s)

    gdje je h*(s) stvarna najjeftinija cijena od s do najbližeg cilja.

    Optimistična heuristika nikad ne precjenjuje stvarnu cijenu do cilja,
    što je nužan uvjet da A* garantira optimalan put.

    Args:
        graph: dict - graf prijelaza
        heuristics: dict - heurističke vrijednosti
    ...
    for state in sorted(all_states):
        # Dohvati heurističku procjenu h(s)
        h_s = heuristics.get(state, 0.0)
        # Izračunaj stvarnu najjeftiniju cijenu h*(s)
        h_s_star = shortest_path_cost(state, goals, graph)

        result = ""
        # Provjera uvjeta: h(s) <= h*(s)
        # Ako je cilj nedostižan (h* = inf), ignoriramo/OK
        if h_s_star == float('inf') or h_s <= h_s_star:
            result = "[OK]"
        else:
            # Heuristika precjenjuje - nije optimistična
            result = "[ERR]"
            is_optimistic = False
...
def check_consistent(graph, heuristics, heuristic_file):
    ...
    Konzistentnost je jači uvjet od optimističnosti. Ako je heuristika
    konzistentna, ona je i optimistična. Konzistentna heuristika garantira
    da A* neće morati ponovo otvarati zatvorena stanja.
    ...
    for state_s in sorted(all_states):
        # Dohvati heurističku vrijednost h(s)
        ...
        for neighbor_t, cost_c in successors:
            # Dohvati heurističku vrijednost h(t)
...
def print_result(alg_name, result, heuristic_file=None):
    """
    Ispisuje rezultat pretraživanja u standardnom formatu.

    Args:
        alg_name: str - ime algoritma (BFS, UCS, A-STAR)
        result: dict - rezultat pretraživanja
    ...
    # Ako rješenje nije pronađeno
    if not result["found"]:
        print("[FOUND_SOLUTION]: no")
        return

    # Ako je rješenje pronađeno, ispiši sve detalje
    print("[FOUND_SOLUTION]: yes")
    print(f"[STATES_VISITED]: {result['visited']}")
    # Dužina puta je broj čvorova
...
if __name__ == "__main__":
    ...
    # Parsiraj argumente u rječnik i skup zastavica
    for i in range(len(args)):
        if args[i].startswith("--"):
            if i + 1 < len(args) and not args[i + 1].startswith("--"):
                # Argument s vrijednošću (npr. --ss istra.txt)
    ...
    # 2. Učitavanje stanja (potrebno za sve modove)
    start_state, goal_states, graph = load_state_space(ss_path)

    # 3. Učitavanje heuristike (potrebno za A* i provjere)
    heuristics = {}
    if h_path:
        heuristics = load_heuristic(h_path)

    # 4. Provjere heuristike (imaju najviši prioritet)
    ...
    # 5. Pokretanje algoritama pretraživanja
    if not alg:
        sys.exit(1)
